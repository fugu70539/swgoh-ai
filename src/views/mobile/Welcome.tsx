...
export default function WelcomeMobile() {
  ...
  const send = () => {
    const t = v.trim();
    // Запрещаем: пустоту, 1 символ, или только спецсимволы/эмодзи
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-white overflow-hidden font-sans">
      ...
      <div className="flex-1 relative w-full">
        ...
        <div className={`absolute inset-0 flex flex-col justify-center px-8 transition-all duration-500 ease-in-out ${st === 'chat' ? 'opacity-0 -translate-y-20 pointer-events-none' : 'opacity-100'}`}>
          <div className="w-full max-w-[440px]">
            ...
            {/* Отступ mt-4 (16px), чтобы подсказки не "липли" к заголовку */}
        ...
        {/* Chat State (Messages) */}
        <div className={`absolute inset-0 pt-28 px-8 overflow-y-auto no-scrollbar transition-opacity duration-500 ${st === 'chat' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      ...
      {/* Input Group - Летает между центром и низом */}
